...
public class MIMdimensionalUnitUtils {
	...
	boolean equalWithinTol(double v1, double v2, double tol)
	{
		return ((v1 >= v2 - tol) && (v1 <= v2 + tol));
	}
	
	public Length lengthMeasureWithUnit(ELength_measure_with_unit e_lmwu) throws SdaiException
	{
		double measure_value = e_lmwu.getValue_component(null, (ELength_measure) null);
		ELength_unit lengthUnit = (ELength_unit) e_lmwu.getUnit_component(null);
		double conversionFactor = getConversionFactorToMM(lengthUnit);
		if (equalWithinTol(conversionFactor, 1.0, 0.0001))
			return new Length(measure_value, LengthUnit.MM);
		else if (equalWithinTol(conversionFactor, 25.4, 0.0001))
			return new Length(measure_value, LengthUnit.INCH);
		else
		{
			throw new IllegalArgumentException("Unsupported length unit: "+e_lmwu);
		}
	}
