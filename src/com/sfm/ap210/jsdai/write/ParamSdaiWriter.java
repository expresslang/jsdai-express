...
import jsdai.SAp210_electronic_assembly_interconnect_and_packaging_design_mim_lf.CMeasure_representation_item$thermal_resistance_measure_with_unit;
...
import com.sfm.ap210.jsdai.param.DerivedSiUnit.ThermalResistanceUnit;
...
public class ParamSdaiWriter {
	...
	ESi_unit getSiUnit(SiUnit u) throws SdaiException
	{
		...
		if (u.base == SiUnit.SiBase.M)
			...
			e_siu = getHenryUnit();
		else if (u.base == SiUnit.SiBase.g)
			e_siu = getGramUnit();
		else
	...
	ESi_unit getGramUnit() throws SdaiException
	{
		ESi_unit e_g_unit = (ESi_unit) outModel.createEntityInstance(CMass_unit$si_unit.class);
		e_g_unit.setName(null, ESi_unit_name.GRAM);		
		return e_g_unit;
	}
	
	ESi_unit getKgUnit() throws SdaiException
	{
		return getSiUnit(new SiUnit(SiBase.g, SiPrefix.kilo));				
	}
	...
	public void addMeasureRepresentationItem(ERepresentation e_r, String name, double value, Unit u) throws SdaiException
	{
		...
		if (u instanceof SiUnit) {
			...
			e_mri = specialMeasureRepresentationItem(name, value, (DerivedSiUnit) u);
			if(e_mri == null) {
				e_mri = genericMeasureRepresentationItem(name, value, (DerivedSiUnit) u);
			}
		}
		if(e_mri == null) {
	...
	public void addRealRepresentationItem(ERepresentation e_r, String name, double value) throws SdaiException
	{
	...
	EMeasure_representation_item specialMeasureRepresentationItem(String name, double value, DerivedSiUnit u) throws SdaiException {
		EMeasure_representation_item e_mri = null;
		if(u instanceof ThermalResistanceUnit) {
			e_mri = (EMeasure_representation_item) outModel.createEntityInstance(CMeasure_representation_item$thermal_resistance_measure_with_unit.class);
	...
	EMeasure_representation_item genericMeasureRepresentationItem(String name, double value, DerivedSiUnit u) throws SdaiException
	{
		EMeasure_representation_item e_mri = null;
	...
	EMeasure_representation_item measureRepresentationItem(String name, double value, SiUnit u) throws SdaiException
	{
		...
		else if (u.base == SiUnit.SiBase.degC || u.base == SiUnit.SiBase.degK)
		{
		...
		{
			e_mri = (EMeasure_representation_item) outModel.createEntityInstance(CMeasure_representation_item$power_measure_with_unit.class);
			e_mri.setValue_component(null, value, (EPower_measure) null);
		}
		else {
